...
public class ProcurementCloseProvider implements ModuleCloseProvider {
    ...
    public ProcurementCloseProvider(CloseBlockerQueryService queries) {
    ...
    private long blockers(String periodId) {
        return queries.dated(periodId, "purchase_orders", "po_date", "status in ('DRAFT','ISSUED','PARTIALLY_RECEIVED')");
    }
    ...
    public Optional<String> getBlockerReason(String periodId) {
        ...
        return count == 0 ? Optional.empty() : Optional.of(count + " purchase order(s) remain unfinished");
    }
