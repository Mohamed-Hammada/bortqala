...
public class ProcurementExcelExporter {
    public byte[] export(List<ProcurementApi.PurchaseOrderResponse> orders,
        ...
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream output = new ByteArrayOutputStream()) {
            ...
            for (var value : orders) {
                ...
                text(row, 0, value.poNumber());
                date(row, 1, value.poDate());
                text(row, 2, value.supplierName());
                text(row, 3, value.currencyCode());
                text(row, 4, value.status());
                number(row, 5, value.totalAmount());
            }
            finish(orderSheet, 6);

            var receiptSheet = workbook.createSheet(arabic ? "إيصالات البضائع" : "Goods Receipts");
            receiptSheet.setRightToLeft(arabic);
            metadata(receiptSheet, arabic, actor);
            ...
            for (var value : receipts) {
                ...
                text(row, 0, value.grnNumber());
                date(row, 1, value.receiptDate());
                text(row, 2, value.supplierName());
                text(row, 3, value.purchaseOrderId());
                text(row, 4, value.status());
            }
            ...
            for (var value : invoices) {
                ...
                text(row, 0, value.invoiceNumber());
                text(row, 1, value.internalReference());
                date(row, 2, value.invoiceDate());
                text(row, 3, value.supplierName());
                text(row, 4, value.currencyCode());
                number(row, 5, value.totalAmount());
                number(row, 6, value.discountAmount());
            ...
            for (var value : payments) {
                ...
                text(row, 0, value.paymentNumber());
                date(row, 1, value.paymentDate());
                text(row, 2, value.supplierName());
    ...
    private void row(org.apache.poi.ss.usermodel.Sheet sheet, int rowIndex, CellStyle style, String[] values) {
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            cell.setCellValue(com.bemo.hr.reporting.infrastructure.ExcelExportSupport.escapeFormula(values[i]));
            cell.setCellStyle(style);
        }
    }

    private void text(org.apache.poi.ss.usermodel.Row row, int column, String value) {
        row.createCell(column).setCellValue(com.bemo.hr.reporting.infrastructure.ExcelExportSupport.escapeFormula(value == null ? "" : value));
    }

    private void number(org.apache.poi.ss.usermodel.Row row, int column, java.math.BigDecimal value) {
        if (value == null) text(row, column, "");
        else row.createCell(column).setCellValue(value.doubleValue());
    }

    private void date(org.apache.poi.ss.usermodel.Row row, int column, long epochMs) {
        row.createCell(column).setCellValue(java.util.Date.from(Instant.ofEpochMilli(epochMs)));
    }

    private void finish(org.apache.poi.ss.usermodel.Sheet sheet, int columns) {
