...
@Entity @Table(name="customer_receipts") @Getter
public class CustomerReceipt {
    ...
    @Column(name="unallocated_amount",nullable=false,precision=19,scale=2) private BigDecimal unallocatedAmount;
    ...
    public CustomerReceipt(String number,String customerId,LocalDate date,String currency,BigDecimal amount,String operation,String actor){
        id=UUID.randomUUID().toString();receiptNumber=number.strip();this.customerId=customerId;receiptDate=date;currencyCode=currency.strip().toUpperCase();
        this.amount=amount;unallocatedAmount=amount;operationId=operation;createdBy=actor;
    }
    public void allocate(BigDecimal value){if(value.signum()<=0||value.compareTo(unallocatedAmount)>0)throw new IllegalArgumentException("Allocation exceeds receipt");unallocatedAmount=unallocatedAmount.subtract(value);}
