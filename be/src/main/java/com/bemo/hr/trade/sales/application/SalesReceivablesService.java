...
public class SalesReceivablesService {
    ...
    public SalesApi.CreditProfileResponse credit(String customerId){requireCustomer(customerId);return creditResponse(creditRepository.findByCustomerId(customerId).orElse(null),customerId);}

    @Transactional
    public SalesApi.CreditProfileResponse updateCredit(String customerId,SalesApi.CreditProfileRequest request,String actor){
        requireCustomer(customerId);CustomerCreditProfile profile=creditRepository.findByCustomerId(customerId).orElseGet(()->new CustomerCreditProfile(customerId));
        profile.update(request.creditLimit(),request.paymentTermsDays(),request.creditHold());creditRepository.save(profile);
    ...
    public void assertCreditAvailable(String customerId,BigDecimal exposure){
        CustomerCreditProfile profile=creditRepository.findByCustomerId(customerId).orElse(null);if(profile==null)return;
        if(profile.isCreditHold())throw error("AR_CUSTOMER_CREDIT_HOLD",HttpStatus.CONFLICT);
        if(profile.getCreditLimit().subtract(invoiceRepository.outstanding(customerId)).compareTo(exposure)<0)throw error("AR_CREDIT_LIMIT_EXCEEDED",HttpStatus.CONFLICT);
    }

    @Transactional
    public SalesApi.InvoiceResponse createInvoice(SalesApi.InvoiceRequest request,String actor){
        if(invoiceRepository.existsByInvoiceNumberIgnoreCase(request.invoiceNumber()))throw error("AR_INVOICE_NUMBER_EXISTS",HttpStatus.CONFLICT);
        requireCustomer(request.customerId());LocalDate date=date(request.invoiceDate());LocalDate due=request.dueDate()>0?date(request.dueDate()):
        ...
        auditService.record("CREATE","CUSTOMER_INVOICE",saved.getId(),actor,"{\"number\":\""+saved.getInvoiceNumber()+"\"}",null);return invoice(saved);
    }
    ...
    public SalesApi.ReceiptResponse recordReceipt(SalesApi.ReceiptRequest request,String actor){
        Optional<CustomerReceipt> replay=receiptRepository.findByOperationId(request.operationId());if(replay.isPresent())return receipt(replay.get());
        if(receiptRepository.existsByReceiptNumberIgnoreCase(request.receiptNumber()))throw error("AR_RECEIPT_NUMBER_EXISTS",HttpStatus.CONFLICT);
        requireCustomer(request.customerId());List<SalesApi.AllocationRequest> requested=request.allocations()==null?List.of():request.allocations();
        if(requested.stream().map(SalesApi.AllocationRequest::invoiceId).distinct().count()!=requested.size())throw error("AR_DUPLICATE_ALLOCATION",HttpStatus.BAD_REQUEST);
    ...
    @Transactional
    public List<SalesApi.CollectionTaskResponse> collections(LocalDate asOf){Map<String,CustomerInvoice> invoices=invoiceRepository.findAllByOrderByInvoiceDateDescCreatedAtDesc().stream().collect(Collectors.toMap(CustomerInvoice::getId,Function.identity()));
        for(CustomerInvoice invoice:invoices.values()){if(invoice.overdue(asOf)&&taskRepository.findByInvoiceId(invoice.getId()).isEmpty())taskRepository.save(new CollectionTask(invoice.getId()));
            if(invoice.getStatus()==CustomerInvoice.Status.PAID)taskRepository.findByInvoiceId(invoice.getId()).ifPresent(CollectionTask::close);}
    ...
    private SalesApi.InvoiceResponse invoice(CustomerInvoice i){return new SalesApi.InvoiceResponse(i.getId(),i.getInvoiceNumber(),i.getCustomerId(),i.getSalesOrderId(),ms(i.getInvoiceDate()),ms(i.getDueDate()),i.getCurrencyCode(),i.getAmount(),i.getOutstandingAmount(),i.getStatus().name(),i.getVersion());}
    private SalesApi.ReceiptResponse receipt(CustomerReceipt r){return new SalesApi.ReceiptResponse(r.getId(),r.getReceiptNumber(),r.getCustomerId(),ms(r.getReceiptDate()),r.getCurrencyCode(),r.getAmount(),r.getUnallocatedAmount(),r.getOperationId(),allocationRepository.findByReceiptId(r.getId()).stream().map(a->new SalesApi.AllocationResponse(a.getInvoiceId(),a.getAmount())).toList());}
    private SalesApi.CollectionTaskResponse task(CollectionTask t,CustomerInvoice i,LocalDate asOf){return new SalesApi.CollectionTaskResponse(t.getId(),t.getInvoiceId(),i==null?"—":i.getInvoiceNumber(),i==null?"":i.getCustomerId(),i==null?BigDecimal.ZERO:i.getOutstandingAmount(),i==null?0:ms(i.getDueDate()),i==null?0:(int)Math.max(0,ChronoUnit.DAYS.between(i.getDueDate(),asOf)),t.getStatus().name(),t.getOwnerUserId(),t.getNextActionDate()==null?0:ms(t.getNextActionDate()),t.getNote(),t.getVersion());}
    private static LocalDate date(long value){return Instant.ofEpochMilli(value).atZone(ZoneOffset.UTC).toLocalDate();}private static long ms(LocalDate value){return value.atStartOfDay(ZoneOffset.UTC).toInstant().toEpochMilli();}
