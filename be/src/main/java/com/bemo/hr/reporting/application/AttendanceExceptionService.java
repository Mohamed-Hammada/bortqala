...
public class AttendanceExceptionService {
    ...
    private static int scaled(int base, int excess) {
        return Math.min(100, base + Math.min(40, excess / 15 * 5));
    }
    ...
    private static int metric(AttendanceException e, DailyAttendanceResult r) {
        if (r == null) return 0;
        return switch (e.getExceptionType()) {
            case LATE -> r.getLateMinutes();
            case EARLY_LEAVE -> r.getEarlyLeaveMinutes();
            case EXCESS_SHIFT -> r.getWorkedMinutes();
            default -> r.getExpectedMinutes();
        };
    }
    ...
    public List<AttendanceExceptionApi.PolicyResponse> policies() {
        return policyRepository.findAllByOrderByPriorityDescEffectiveFromDesc().stream().map(this::policy).toList();
    }
    ...
    public AttendanceExceptionApi.BulkResult apply(String reportId, AttendanceExceptionApi.BulkRequest request, String actor) {
        ...
        for (AttendanceException exception : selected) {
            if (exception.replay(request.operationId())) {
                replayed++;
                continue;
            }
            if (exception.getStatus() != AttendanceExceptionStatus.OPEN) {
                skipped++;
                continue;
            }
        ...
        report.updateUnresolvedCount(unresolved);
    ...
    private List<Candidate> candidates(DailyAttendanceResult r, AttendancePolicy p) {
        ...
        if (r.getStatus() == DailyStatus.NO_PUNCH)
            out.add(new Candidate(AttendanceExceptionType.NO_PUNCH, p.getMissingPunchScore(), "attendance.exception.noPunch"));
        if (r.getStatus() == DailyStatus.SINGLE_PUNCH)
            out.add(new Candidate(AttendanceExceptionType.SINGLE_PUNCH, p.getSinglePunchScore(), "attendance.exception.singlePunch"));
        if (r.getStatus() == DailyStatus.MISSING_SCHEDULE)
            out.add(new Candidate(AttendanceExceptionType.MISSING_SCHEDULE, 100, "attendance.exception.missingSchedule"));
        if (r.getLateMinutes() > p.getLateThresholdMinutes())
            out.add(new Candidate(AttendanceExceptionType.LATE, scaled(p.getLateScore(), r.getLateMinutes() - p.getLateThresholdMinutes()), "attendance.exception.late"));
        if (r.getEarlyLeaveMinutes() > p.getEarlyThresholdMinutes())
            out.add(new Candidate(AttendanceExceptionType.EARLY_LEAVE, scaled(p.getEarlyScore(), r.getEarlyLeaveMinutes() - p.getEarlyThresholdMinutes()), "attendance.exception.earlyLeave"));
        if (r.getWorkedMinutes() > p.getMaxShiftMinutes())
            out.add(new Candidate(AttendanceExceptionType.EXCESS_SHIFT, 80, "attendance.exception.excessShift"));
        return out;
    }
    ...
    private void applyResolution(DailyAttendanceResult r, AttendanceExceptionResolution resolution, String reason, String actor) {
        switch (resolution) {
            ...
            case ACCEPT -> {
                if (r.isBlocking())
                    r.decide(r.getStatus() == DailyStatus.NO_PUNCH ? AttendanceDecision.ABSENCE : AttendanceDecision.NORMAL_DAY, r.getStatus() == DailyStatus.NO_PUNCH ? 0 : r.getExpectedMinutes(), reason, actor);
            }
            case IGNORE -> {
            }
        }
    ...
    private AttendanceReport requireReport(String id) {
        return reportRepository.findById(id).orElseThrow(() -> error("RPT_NOT_FOUND", HttpStatus.NOT_FOUND));
    }

    private AttendanceReport requireEditable(String id) {
        AttendanceReport r = requireReport(id);
        if (r.getStatus() != ReportStatus.IN_REVIEW)
            throw error("ATTENDANCE_EXCEPTION_PERIOD_LOCKED", HttpStatus.CONFLICT);
        return r;
    }

    private void validatePolicy(AttendanceExceptionApi.PolicyRequest r) {
        if (r.scopeType() != AttendancePolicyScope.TENANT && (r.scopeId() == null || r.scopeId().isBlank()) || r.effectiveTo() != null && r.effectiveTo().isBefore(r.effectiveFrom()))
            throw error("ATTENDANCE_POLICY_INVALID", HttpStatus.BAD_REQUEST);
    }

    private AttendanceExceptionApi.PolicyResponse policy(AttendancePolicy p) {
        return new AttendanceExceptionApi.PolicyResponse(p.getId(), p.getName(), p.getScopeType(), p.getScopeId(), p.getEffectiveFrom(), p.getEffectiveTo(), p.getPriority(), p.getLateThresholdMinutes(), p.getEarlyThresholdMinutes(), p.getMaxShiftMinutes(), p.getMissingPunchScore(), p.getSinglePunchScore(), p.getLateScore(), p.getEarlyScore(), p.getPayrollBlockScore(), p.isActive(), p.getVersion());
    }

    private AttendanceExceptionApi.ExceptionView view(AttendanceException e, DailyAttendanceResult r) {
        return new AttendanceExceptionApi.ExceptionView(e.getId(), e.getReportId(), e.getDailyResultId(), e.getEmployeeId(), r == null ? "—" : r.getEmployeeName(), e.getCategoryId(), r == null ? "—" : r.getCategoryName(), e.getWorkDate(), e.getExceptionType(), e.getScore(), metric(e, r), e.getExplanationKey(), e.getPolicyId(), e.getPolicyName(), e.getPolicyVersion(), e.getPolicySnapshotJson(), e.getPolicyScope(), e.isPayrollBlocking(), e.getStatus(), e.getResolution(), e.getReason(), e.getVersion());
    }
