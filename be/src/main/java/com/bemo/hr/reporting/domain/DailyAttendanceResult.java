...
public class DailyAttendanceResult {
    ...
    @Column(name = "first_punch") private Instant firstPunch;
    @Column(name = "last_punch") private Instant lastPunch;
    @Column(name = "punch_count", nullable = false) private int punchCount;
    @Column(name = "expected_minutes", nullable = false) private int expectedMinutes;
    @Column(name = "worked_minutes", nullable = false) private int workedMinutes;
    @Column(name = "manual_worked_minutes") private Integer manualWorkedMinutes;
    ...
    @Enumerated(EnumType.STRING) @Column(nullable = false, length = 30) private DailyStatus status;
    @Column(length = 255) private String warning;
    @Enumerated(EnumType.STRING) @Column(length = 30) private AttendanceDecision decision;
    ...
    public DailyAttendanceResult(String reportId, String employeeId, String categoryId, LocalDate workDate,
        ...
        this.id = UUID.randomUUID().toString(); this.reportId = reportId; this.employeeId = employeeId;
        this.categoryId = categoryId; this.workDate = workDate; this.employeeCode = employeeCode;
        this.employeeName = employeeName; this.categoryName = categoryName; this.firstPunch = firstPunch;
        this.lastPunch = lastPunch; this.punchCount = punchCount; this.expectedMinutes = expectedMinutes;
        this.workedMinutes = workedMinutes; this.lateMinutes = lateMinutes; this.earlyLeaveMinutes = earlyLeaveMinutes;
        this.overtimeMinutes = overtimeMinutes; this.status = status; this.warning = warning; this.ruleVersion = ruleVersion;
    }
    ...
    public String getCategoryId() { return categoryId; } public LocalDate getWorkDate() { return workDate; }
    public String getEmployeeCode() { return employeeCode; } public String getEmployeeName() { return employeeName; }
    public String getCategoryName() { return categoryName; } public Instant getFirstPunch() { return firstPunch; }
    public Instant getLastPunch() { return lastPunch; } public int getPunchCount() { return punchCount; }
    public int getExpectedMinutes() { return expectedMinutes; } public int getWorkedMinutes() { return workedMinutes; }
    public Integer getManualWorkedMinutes() { return manualWorkedMinutes; }
    public int getEffectiveWorkedMinutes() { return manualWorkedMinutes == null ? workedMinutes : manualWorkedMinutes; }
    public int getLateMinutes() { return lateMinutes; } public int getEarlyLeaveMinutes() { return earlyLeaveMinutes; }
    public int getOvertimeMinutes() { return overtimeMinutes; } public DailyStatus getStatus() { return status; }
