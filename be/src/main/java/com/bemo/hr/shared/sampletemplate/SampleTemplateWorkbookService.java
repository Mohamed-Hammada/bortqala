...
public class SampleTemplateWorkbookService {
    ...
    private static void autoSize(Sheet sheet, int count) {
        for (int i = 0; i < count; i++) {
            ...
            sheet.setColumnWidth(i, Math.min(sheet.getColumnWidth(i) + 768, 14000));
        }
    }
    ...
    public byte[] create(SampleTemplateCatalog.Template template) {
        ...
        try (XSSFWorkbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            for (int i = 0; i < template.columns().size(); i++) {
                ...
                cell.setCellValue(template.columns().get(i).en());
            ...
            for (int i = 0; i < labels.length; i++) {
                Cell cell = ih.createCell(i);
                cell.setCellValue(labels[i]);
                cell.setCellStyle(header);
            }
            for (int r = 0; r < template.columns().size(); r++) {
                var col = template.columns().get(r);
                Row row = info.createRow(r + 1);
                row.createCell(0).setCellValue(col.en());
                row.createCell(1).setCellValue(col.ar());
                row.createCell(2).setCellValue(col.required() ? "Yes / نعم" : "No / لا");
                row.createCell(3).setCellValue(col.type());
                row.createCell(4).setCellValue(col.accepted());
                row.createCell(5).setCellValue(col.notes());
            }
