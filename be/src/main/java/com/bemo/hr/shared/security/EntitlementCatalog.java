...
public class EntitlementCatalog {
    ...
    public List<Module> modules(){return features.stream().collect(java.util.stream.Collectors.groupingBy(Feature::moduleKey,LinkedHashMap::new,java.util.stream.Collectors.toList())).entrySet().stream().map(e->new Module(e.getKey(),List.copyOf(e.getValue()))).toList();}
    public Optional<Feature> feature(String key){return Optional.ofNullable(byKey.get(key));}
    public boolean defaultEnabled(String key){return feature(key).map(Feature::defaultEnabled).orElse(false);}
    public Map<String,Boolean> defaults(){return features.stream().collect(java.util.stream.Collectors.toMap(Feature::key,Feature::defaultEnabled));}
    public Set<String> dependents(String key){return features.stream().filter(f->f.dependencies().contains(key)).map(Feature::key).collect(java.util.stream.Collectors.toSet());}
    public Optional<String> requiredFeature(String uri){return features.stream().flatMap(f->f.apiPrefixes().stream().map(p->Map.entry(p,f.key()))).filter(e->uri.startsWith(e.getKey())).max(Comparator.comparingInt(e->e.getKey().length())).map(Map.Entry::getValue);}
    private static Feature f(String key,String module,Set<String> dependencies,String... prefixes){return f(true,key,module,dependencies,prefixes);}
    private static Feature f(boolean enabled,String key,String module,Set<String> dependencies,String... prefixes){return new Feature(key,module,enabled,dependencies,List.of(prefixes));}
}
