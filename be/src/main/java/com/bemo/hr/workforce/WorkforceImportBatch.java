...
public class WorkforceImportBatch {
    ...
    @Lob @Column(name = "original_file", nullable = false) private byte[] originalFile;
    @Column(nullable = false, length = 30) private String status;
    @Column(name = "headers_text", length = 2000) private String headersText;
    @Column(name = "column_mapping", length = 2000) private String columnMapping;
    ...
    public void map(String columnMapping) { this.columnMapping = columnMapping; this.status = "MAPPED"; }
    public void validated(int totalRows, int validRows, int invalidRows) {
        this.totalRows = totalRows; this.validRows = validRows; this.invalidRows = invalidRows;
        this.status = invalidRows == 0 ? "READY" : "VALIDATED";
    }
    public void readyWithAcceptedErrors() { this.status = "READY"; }
    public void imported(String operationId, int importedRows) {
        this.operationId = operationId; this.importedRows = importedRows; this.status = "IMPORTED"; this.importedAt = Instant.now();
    }
    public void reversed(String actor) { this.status = "REVERSED"; this.reversedAt = Instant.now(); this.reversedBy = actor; }
