...
@Entity
@Table(name = "approval_instance_steps")
@Getter
public class ApprovalInstanceStep {
    ...
    @Column(name = "source_step_id", nullable = false, length = 36)
    ...
    @Column(name = "required_user_id", length = 100)
    ...
    public ApprovalInstanceStep(String instanceId, ApprovalWorkflowStep source, int effectiveOrder) {
        ...
        stepCode = source.getStepCode();
        name = source.getName();
        requiredRole = source.getRequiredRole();
        requiredUserId = source.getRequiredUserId();
        minimumApprovals = source.getMinimumApprovals();
        allowSelfApproval = source.isAllowSelfApproval();
        escalationHours = source.getEscalationHours();
        decisionPolicy = source.getDecisionPolicy();
        createdAt = Instant.now();
    }

    public void reassign(String userId, String actor, String reason) {
        requiredUserId = userId.strip();
        requiredRole = null;
        reassignedBy = actor;
        reassignedAt = Instant.now();
        reassignmentReason = reason.strip();
    }
}
