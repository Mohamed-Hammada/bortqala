...
public class DemoScenarioDataService {
    ...
    public void ensureDemoScenarios() {
        var administration = category("ADMINISTRATION"); administration.configureAdvanceEligibility(true);
        var accounting = category("ACCOUNTING"); accounting.configureAdvanceEligibility(true);
    ...
    private void seedPunches(Employee admin, Employee accountant, Employee guard) {
        ...
        rows.add(new PunchSeed(accountant, workDays.get(0), LocalTime.of(7, 58))); rows.add(new PunchSeed(accountant, workDays.get(0), LocalTime.of(18, 9)));
        rows.add(new PunchSeed(guard, workDays.get(0), LocalTime.of(8, 1)));
        rows.add(new PunchSeed(admin, workDays.get(1), LocalTime.of(8, 35))); rows.add(new PunchSeed(admin, workDays.get(1), LocalTime.of(17, 3)));
    ...
    private BusinessParty party(String code,String name,String type){return businessPartyRepository.findByCodeIgnoreCase(code).orElseGet(()->businessPartyRepository.save(new BusinessParty(code,name,type,null,null,"بيانات تجريبية",true)));}
