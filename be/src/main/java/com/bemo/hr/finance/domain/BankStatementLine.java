...
public class BankStatementLine {
    ...
    public BigDecimal remainingAmount() {
        return amount.abs().subtract(matchedAmount);
    }

    public void addMatch(BigDecimal value) {
        matchedAmount = matchedAmount.add(value.abs());
        status = remainingAmount().signum() == 0 ? Status.MATCHED : Status.PARTIAL;
    }

    public void reverseMatch(BigDecimal value) {
        matchedAmount = matchedAmount.subtract(value.abs()).max(BigDecimal.ZERO);
        status = matchedAmount.signum() == 0 ? Status.UNMATCHED : Status.PARTIAL;
    }
