...
public class GeneralLedgerReportService {
    ...
    public List<Row> detail(LocalDate from, LocalDate to, String accountId) {
        ...
        for (JournalEntry e : entries.findByStatusOrderByEntryDateDesc(JournalEntry.Status.POSTED).stream().filter(x -> !x.getEntryDate().isBefore(from) && !x.getEntryDate().isAfter(to)).sorted(Comparator.comparing(JournalEntry::getEntryDate).thenComparing(JournalEntry::getEntryNumber)).toList()) {
            for (JournalEntryLine l : lines.findByJournalEntryId(e.getId())) {
                if (accountId != null && !accountId.equals(l.getAccountId())) continue;
                running = running.add(l.getDebit()).subtract(l.getCredit());
                Account a = map.get(l.getAccountId());
                out.add(new Row(e.getId(), e.getEntryNumber(), e.getEntryDate(), l.getAccountId(), a == null ? l.getAccountId() : a.getCode(), e.getReference(), l.getDebit(), l.getCredit(), running));
            }
        }
        return out;
    }
