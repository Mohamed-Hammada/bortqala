...
public class JournalDimensionReportService {
    ...
    public JournalDimensionReportService(JdbcTemplate jdbcTemplate) { this.jdbcTemplate = jdbcTemplate; }
    public record DimensionSummary(String costCenterId, String projectId, String departmentId, BigDecimal debit, BigDecimal credit, BigDecimal balance) {}
    public List<DimensionSummary> summarize(LocalDate from, LocalDate to, String costCenterId, String projectId, String departmentId) {
        ...
        if (costCenterId != null) { sql.append(" and d.cost_center_id=?"); args.add(costCenterId); }
        if (projectId != null) { sql.append(" and d.project_id=?"); args.add(projectId); }
        ...
        return jdbcTemplate.query(sql.toString(), (rs, n) -> { BigDecimal debit=rs.getBigDecimal(4), credit=rs.getBigDecimal(5); return new DimensionSummary(rs.getString(1),rs.getString(2),rs.getString(3),debit,credit,debit.subtract(credit)); }, args.toArray());
    }
}
