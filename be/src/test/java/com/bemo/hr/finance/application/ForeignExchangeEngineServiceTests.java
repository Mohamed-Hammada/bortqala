...
class ForeignExchangeEngineServiceTests {
    ...
    void postsGainAndLossReplaySafelyAndReversesLinkedJournal() {
        FxPostingRepository postings=mock(FxPostingRepository.class); SubledgerPostingService postingService=mock(SubledgerPostingService.class); FiscalPeriodGuard guard=mock(FiscalPeriodGuard.class);
        FiscalPeriod period=new FiscalPeriod(2026,8,"August",LocalDate.of(2026,8,1),LocalDate.of(2026,8,31),FiscalPeriod.Status.OPEN);
        when(guard.requireOpen(LocalDate.of(2026,8,31))).thenReturn(period); when(postings.save(any())).thenAnswer(i->i.getArgument(0));
        JournalEntry journal=new JournalEntry("FX-1",LocalDate.of(2026,8,31),"FX","FX",null);when(postingService.postSubledgerEvent(anyString(),anyString(),anyString(),anyString(),anyString(),any(),anyString(),any(),any(),anyString())).thenReturn(journal);
        ...
        assertThat(gain.getGainLossAmount()).isEqualByComparingTo("200"); assertThat(gain.getRateSource()).isEqualTo("CB-EGYPT");
        when(postings.findByOperationId("fx-1")).thenReturn(java.util.Optional.of(gain)); assertThat(service.post(FxPosting.Type.UNREALIZED,"inv-1",BigDecimal.ONE,BigDecimal.ONE,BigDecimal.ONE,"X",LocalDate.now(),"fx-1")).isSameAs(gain);
    }
}
