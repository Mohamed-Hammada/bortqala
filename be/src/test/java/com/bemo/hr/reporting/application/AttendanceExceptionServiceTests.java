...
class AttendanceExceptionServiceTests {
    ...
    @Mock AttendanceExceptionRepository exceptionRepository;
    @Mock AttendanceReportRepository reportRepository;
    @Mock DailyAttendanceResultRepository resultRepository;
    @Mock AuditService auditService;
    @InjectMocks AttendanceExceptionService service;

    @Test void employeePolicyWinsOverCategoryAndTenantAndExplainsTheScore() {
    ...
    @Test void manualOverrideUpdatesTheDailyEvidenceAndClearsTheBlocker() {
    ...
    @Test void lockedReportRejectsManualOverrides() {
        AttendanceReport report = report(0); report.approve("admin");
        when(reportRepository.findById(report.getId())).thenReturn(Optional.of(report));
    ...
    @Test void approvedPayrollIsBlockedWhileCriticalExceptionsRemainOpen() {
        AttendanceReport report = report(0); report.approve("admin");
        when(reportRepository.findById(report.getId())).thenReturn(Optional.of(report));
    ...
    @Test void crossMidnightCheckoutBelongsToThePreviousWorkday() {
    ...
    private AttendanceReport report(int unresolved) {
        ...
        report.startReview(unresolved); return report;
    }
