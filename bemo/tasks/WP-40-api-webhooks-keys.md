...
- [ ] AC-1 Key with `invoices:read` reads invoices but 403s creating customer without scope; wrong key → generic 401 (no enumeration).
- [ ] AC-2 Rate limit trips at configured N/min returning 429+Retry-After; counter resets next minute (fake clock).
- [ ] AC-3 invoice.paid delivery body matches published schema snapshot; invalid-signature receiver documented test shows rejection guidance; 5 failures → dead status visible in UI.
- [ ] AC-4 Full key displayed exactly once; only hash persisted (DB inspection test); revocation effective immediately incl. in-flight? (documented: next request rejected).
- [ ] AC-5 Tenant isolation: key from app A cannot read app B even with same key string replayed (context binding test).
