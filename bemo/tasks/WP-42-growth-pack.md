...
- [x] AC-1 Spend 500 @ rule 1pt/100 → +5 points ledger row with rule snapshot; redeem 200 of them at cap 50% of a 300 sale → discount line exactly 150.
- [x] AC-2 Points expire after configured months via job; expired rows EXPIRE-typed, balance never negative in any interleaving.
- [x] AC-3 Membership renewing: invoice created in grace window; unpaid past grace → EXPIRED; cancel stops future invoices immediately.
- [x] AC-4 Referral rewards fire ONCE per referred party at first posted sale.
- [x] AC-5 All balances are derived-from-ledger verifiable.

## Deliverables Summary
- **Database Schema**: `loyalty_accounts`, `loyalty_ledger`, `membership_plans`, `member_subscriptions`, `referrals` (Liquibase `v381`, `v382`).
- **Backend Architecture**: Package `com.bemo.hr.growth` (`LoyaltyService`, `MembershipService`, `ReferralService`, `GrowthPackController`).
- **Frontend Architecture**: Loyalty & Subscriptions components in party details and settings (`SubscriptionSettingsComponent`).
