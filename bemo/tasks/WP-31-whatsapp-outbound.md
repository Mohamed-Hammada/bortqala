...
- [ ] AC-1 Provider NONE: events queue as NO_CONSENT-style skipped with reason, zero external calls (mock server asserts). — **PARTIAL**: NONE path still early-returns silently (zero external calls holds) — a NO_CONSENT-style skipped log row for NONE is not written.
- [ ] AC-2 Payroll marked PAID for 3 employees → exactly 3 QUEUED → SENT with provider ids; status webhook updates DELIVERED; failure path records error and retry respects backoff (fake clock). — **PARTIAL**: only `NoOpWhatsAppSender` exists (throws `WA_PROVIDER_OFF`); no provider ids/webhook/retry path. Consent-path tests (active consent → SENT via 5-arg `sendTemplate`) added.
- [x] AC-3 Employee without consent is never sent (log proves NO_CONSENT) even when event fires. — **MET**: consent registry wired into `enqueuePayrollPayslip` (`hasConsent` on `withdrawnAt IS NULL` + `hr.whatsapp.consent.purpose`); missing/withdrawn consent → `NO_CONSENT` log row, sender never invoked. Tests: `enqueuePayrollPayslip_withoutConsent_marksNoConsentAndNeverSends`, `..._withActiveConsent_sends`, `..._withdrawnConsent_doesNotSend`.
- [ ] AC-4 Template params render correct Arabic employee name/amount in payload snapshot test; test-send delivers to configured number in staging. — **PARTIAL**: payload snapshot/test-send impossible with NoOp sender.
- [x] AC-5 Duplicate event (payroll re-mark) does not double-send (dedupe key employee+event+period). — **MET**: dedupe key now `PAYSLIP:{employeeId}:{appId}:{period}` (period-aware 6-arg overload; 5-arg delegates with `null`); `enqueuePayrollPayslip_withPeriod_usesPeriodScopedDedupeKey` test green.
