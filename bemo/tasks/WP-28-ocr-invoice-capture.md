...
- [ ] AC-1 With provider NONE: upload returns translated not-configured and UI never shows broken state.
- [ ] AC-2 Golden-image fixture (printed invoice) extracts supplier/no/date/≥2 lines through mock provider; confidence badges render per thresholds.
- [ ] AC-3 Nothing hits inventory/partner-ledger until user converts AND the created record is a DRAFT requiring normal confirm flow (ledger-zero assertion).
- [ ] AC-4 Provider failure surfaces `OCR_PROVIDER_FAILED` with retry; job row keeps image for reprocess.
- [ ] AC-5 API key only ever from env/config — code-review grep test asserts no literal secrets; oversize/bad-type uploads rejected like REM-005 rules.
