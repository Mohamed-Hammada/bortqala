...
- [x] AC-3 Rule-state chip renders correctly on review rows in both locales; gates green. — **MET**: `ruleState(row)` computed chip on every undecided SINGLE_PUNCH row — non-blocking categories (singlePunchCounts=true) render `review.ruleAutoAttended` (auto-attended), blocking ones render `review.ruleBlocking`; both locales present; spec added in `report-review.page.spec.ts` (banner test sets `fixturePage.filter.set('ALL')` since default `UNRESOLVED` filter hides non-blocking rows).
