...
**Final result: 46/46 tests pass, in ~2m20s.** Every class in the 8-class Testcontainers-gated suite passes cleanly against a real `postgres:17-alpine` instance, including all three real payment-concurrency race tests and the full `PunchSourceIdentityMigrationTests`/`LiquibaseUpgradePathTests` migration-replay verification. Getting there took several more rounds of fixing — two more genuine, previously-never-verified test-assertion bugs in `PunchSourceIdentityMigrationTests` itself (see "Historical migration-replay conflict — now fully resolved" below) — each caught only because this was the first time this specific test had ever executed against a real database.
...
#### Historical migration-replay conflict — investigated and fully resolved

`test-v92-pre.changelog-master.yaml` (the frozen "schema state right before v92" snapshot this test replays before applying the real v92 migration) includes 25 individual historical migration files by path. A prior, unrelated commit (`6203ea0`, "master translation file" — a large-scale consolidation of many individual per-version translation-insert changesets into one master CSV) deleted 8 of those 25 files outright, since they were superseded by the new consolidated file for all *current* purposes. That consolidation never accounted for this "-pre" snapshot fixture, which needs the *exact historical files*, not their modern replacement, to faithfully replay history. This had silently broken this test since that commit — invisible until now because this test is gated behind `-PskipDockerTests` and had never run to completion before this pass.

**Fix 1 — missing files:** all 8 deleted YAML changesets, plus the 8 CSV data files they in turn reference (a second layer of the same deletion), were restored byte-for-byte from git history (`git show 6203ea0^:<path>`) into `be/src/test/resources/db/changelog/...` — a test-only location, not touching any production changelog. This fully unblocked `PunchSourceIdentityMigrationTests`' sibling, `LiquibaseUpgradePathTests` (which now passes), and got the replay from "file not found" to actually applying real Liquibase changesets — but then hit a second, deeper layer of the same root cause.

**Fix 2 — the real second source, found:** the replay then failed with `PSQLException: duplicate key value violates unique constraint "uq_translations_key_locale" — Key (translation_key, locale)=(appShell.serverChecking, ar-EG) already exists`. The initial search (limited to the 25 top-level includes) found this key only once and could not locate a second source. Recursively resolving the *entire* include chain (81 files, not just the 25 top-level ones) found it: `20260724_v3_i18n_translations.yaml` — an early, never-deleted changeset still in the chain — loads `db/changelog/data/insert/files/translations.csv`, a single file that has grown in place over the project's life (not a per-version snapshot). The 2026-09-01 consolidation commit *edited this same file*, folding in the content of the 8 deleted per-version files — so replaying "v3" today loads *today's* full catalog (including `appShell.serverChecking`), which then collides with the very v70/v73/etc. changesets restored in Fix 1 when they try to insert the same rows again.

**Fixed:** added one test-only changeset (`test-v92-pre_dedupe_consolidated_translations.yaml`, included in `test-v92-pre.changelog-master.yaml` immediately after the `v1_v67` baseline and before the restored per-version files) that deletes exactly the 990 `(translation_key, locale)` rows the 8 restored files are about to re-insert — generated programmatically from their exact content, not guessed. This makes the replay order historically faithful: v3 loads the catalog as it existed *at v3's original insertion point* (minus the rows this changeset removes, which the later changesets then correctly re-add), rather than colliding with today's already-consolidated copy. `translations.csv` itself was never modified — only a new, test-only cleanup step was inserted into the test-only replay changelog.

**Fix 3 & 4 — two more test-assertion bugs, found once the replay finally ran to completion:** with the schema/data conflicts resolved, the migration itself succeeded for the first time, surfacing two further pre-existing bugs — in the *test's own assertions*, not the production migration:
- `assertSourceCatalog` expected the normalized source name `"gate one"` (a space) for input `"  Gate One  "`. The real, intentional v92 SQL (`LOWER(REGEXP_REPLACE(BTRIM(name), '\s+', '_'))`) normalizes internal whitespace to an **underscore**, not a space — confirmed both by reading the migration's own SQL and by the *sibling* test (`deviceNamesThatDifferOnlyByInnerWhitespaceResolveToOneSource`, already passing) which specifically tests that `"Gate_One"` and `"  Gate  One  "` collapse to the *same* identity, only possible if underscore is the real normalized form. Fixed the assertion to `"gate_one"`.
- `assertConstraints`'s two foreign-key checks selected `constraint_name` from `information_schema.table_constraints` with no `ORDER BY`, then asserted the *first* row was the specific FK it cared about — but `punch_records` and `import_batches` each have more than one FK, and Postgres returned them in a real but unspecified order, so the first row was `fk_punch_records_batch_id`/`fk_import_batches_app_id`, not the intended `fk_punch_records_source`/`fk_import_batches_source`. Fixed both queries to filter directly by the exact constraint name (which the test already knew), rather than relying on unordered row position.

Both were assertions that had simply never been evaluated against a real database before — proof, not merely a claim, that a compiling test file with plausible-looking assertions is not evidence those assertions were ever correct.

**Classification: `FIXED`.** All 46 tests in the 8-class real-PostgreSQL suite pass, including both `PunchSourceIdentityMigrationTests` methods and `LiquibaseUpgradePathTests`.
...
**`POSTGRESQL: VERIFIED`** — superseding §0.5's `UNVERIFIED`. The full Testcontainers/PostgreSQL-gated suite (8 classes, all classes previously excluded by `-PskipDockerTests` across every prior pass in this document) has now actually executed against a real `postgres:17-alpine` container: **46/46 pass**, including all three real payment-concurrency race tests, the punch-source-identity concurrency race test, the reporting bulk-decision concurrency test, the workforce-import concurrency test, and the full `LiquibaseUpgradePathTests`/`PunchSourceIdentityMigrationTests` migration-replay verification.
...
**Docker/PostgreSQL:** [TASK-05 UPDATE, §0.7] `VERIFIED` — superseding the `UNVERIFIED` status this row originally reported. `./gradlew test -PskipAot --tests <8 Testcontainers-gated classes>` was actually executed against a real `postgres:17-alpine` container once Docker became available later the same day: **46/46 tests pass** (see §0.7 for the investigation trail, including two genuine test-assertion bugs in `PunchSourceIdentityMigrationTests` that had never been evaluated against a real database before). This is the same suite every prior pass in this document reported as `1 skipped`/`UNVERIFIED`, run to completion for the first time. Every other backend test in this document, in every pass, executed against H2 — this is the first and only PostgreSQL-backed evidence in this remediation's history.
...
| PostgreSQL-specific behavior (locking, index usage, `MODE=PostgreSQL` compatibility edge cases) | **UNVERIFIED → VERIFIED** | 8-class real-Postgres Testcontainers suite, §0.7 | 46/46 pass against real `postgres:17-alpine`, including full migration-replay verification |
...
| `PunchSourceIdentityMigrationTests` — historical migration-replay conflict (translation-key duplicate) + two unordered-row/wrong-normalization test-assertion bugs | **FIXED** | `PunchSourceIdentityMigrationTests` (both methods) | 16 genuinely-deleted historical files restored from git history, one new test-only dedup changeset added, two incorrect test assertions corrected (see §0.7) — 46/46 in the full suite, including this class |
...
| [§0.7 NEW] `PunchSourceIdentityMigrationTests` historical migration-replay conflict + 2 test-assertion bugs | Not previously identified — this test had never run to completion before | **FIXED** — see §0.7 |
...
6. ~~PostgreSQL/Testcontainers verification remains environmentally blocked.~~ **[§0.7 UPDATE: CLOSED]** — Docker became available later the same day; the full 8-class Testcontainers-gated suite was run against real `postgres:17-alpine`: **46/46 pass**. (Item 8 below, the one remaining split-out precision gap, is also now closed.)
...
8. ~~`ExecutiveCockpitTarget`/`ExecutiveKpiSnapshot` concurrency (§0.3's target-creation race and snapshot upsert) still has no PostgreSQL-backed run.~~ **[§0.8 UPDATE: CLOSED, same day]** — a new class, `be/src/test/java/com/bemo/hr/analytics/api/ExecutiveAnalyticsAuthorizationPostgresIntegrationTests.java`, extends `PostgresIntegrationTest` and proves both mechanisms directly against real `postgres:17-alpine`: real HTTP through the real Spring Security filter chain, a real two-thread `CyclicBarrier` race for the target-creation test, a direct real-repository read afterward. 2/2 pass; no production defect exposed. The existing H2 tests in `ExecutiveAnalyticsAuthorizationIntegrationTests` were left unchanged (still pass) — this is additive coverage. The full 9-class real-Postgres suite (48 tests) passes together, and the full backend H2 regression suite (`./gradlew test -PskipDockerTests`) was re-run afterward and remains `BUILD SUCCESSFUL` — no regression. The new class was added to `build.gradle`'s `skipDockerTests` exclusion list, confirmed by re-running `-PskipDockerTests` and verifying no Testcontainers activity was attempted. See `docs/PRODUCTION_READINESS_AUDIT_2026-09-08.md` for the full evidence trail.
9. ~~`PunchSourceIdentityMigrationTests` remains genuinely broken.~~ **[§0.7 UPDATE: CLOSED]** — the root cause (a mutable, continuously-growing `translations.csv` referenced by an early, never-deleted changeset, colliding with the 8 restored per-version files) was fully isolated by recursively resolving the entire 81-file include chain, not just the 25 top-level includes. Fixed with one new test-only dedup changeset plus two corrected test assertions (a wrong expected normalization, an unordered-row-position assumption); see §0.7. All 46/46 tests in the suite pass.
...
**PostgreSQL/Testcontainers being entirely unavailable — closed.** Docker became available, and §0.7 ran the full 8-class Testcontainers-gated suite against a real `postgres:17-alpine` instance for the first time in this remediation's history, ending at **46/46 passing**. That run itself found and fixed a **critical, previously-invisible bug** that would have prevented the application from starting at all against real PostgreSQL (`@Lob`→`oid` schema-validation mismatch, §0.7) — concrete vindication of this document's repeated "H2 green is not sufficient" warning — plus a real concurrency bug, three payment-test fixture gaps, and a historical migration-replay conflict with two further test-assertion bugs, all fixed and verified.

**The one remaining precision gap — Executive Analytics' own concurrency tests being H2-only — closed the same day (§0.8).** `ExecutiveAnalyticsAuthorizationPostgresIntegrationTests` proves both the snapshot-upsert and target-creation-race mechanisms directly against real PostgreSQL: real HTTP, a real two-thread race, a direct real-repository read, no mocks. 2/2 pass; no production defect exposed. The full 9-class real-Postgres suite (48 tests) passes together, and the full H2 regression suite remains green.

No unresolved production-blocking issue remains. Two accepted, by-design, non-blocking scope decisions remain, not defects:
- Gross margin/COGS and AP aging remain entirely unattributable at branch scope by honest design choice — correct behavior, not communicated to end users via a UI affordance yet.
- The remaining branch-scoped KPIs beyond headcount/cash (POS revenue, payroll, projects, inventory) still rely on Mockito-level proof rather than real HTTP fixtures, by documented scope decision (§0.1).

Recommendation: ship this. Optional, non-blocking follow-ups: (a) add the "not available at branch level" UI affordance for gross margin/COGS and AP aging, (b) treat the two-KPI-real-HTTP-fixture scope decision in §0.1 as a follow-up if broader real-fixture coverage is ever required.
