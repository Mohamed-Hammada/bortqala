...
6. ~~PostgreSQL/Testcontainers verification remains environmentally blocked.~~ **[§0.7 UPDATE: CLOSED]** — Docker became available later the same day; the full 8-class Testcontainers-gated suite was run against real `postgres:17-alpine`: **46/46 pass**. The Executive-Analytics-specific concurrency tests this remediation is primarily about (target creation race, snapshot upsert) are not in this 8-class suite and remain H2-only — see item 8 below, newly split out from this item for precision.
...
Recommendation: proceed with this fix. Before wide release to owners who will make real financial decisions from branch-filtered figures: (a) convert or duplicate `ExecutiveAnalyticsAuthorizationIntegrationTests`'s concurrency tests to run against real PostgreSQL now that Testcontainers access is confirmed available in this environment, (b) add the "not available at branch level" UI affordance for gross margin/COGS and AP aging, (c) treat the two-KPI-real-HTTP-fixture scope decision in §0.1 as a follow-up if broader real-fixture coverage is required.
