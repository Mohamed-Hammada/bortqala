...
> **UPDATE (2026-09-07, Critical Engineering Remediation):** This defect is now fixed. `getFinancialPosition(String, Long)` filters ledger entries to `occurredAt <= asOfDate` before computing both the balance and the aging buckets; `getAgingReport` threads the parameter through. Regression tests added: `asOfDateExcludesEntriesAfterTheCutoff`, `agingBucketsAreRelativeToTheRequestedAsOfDate`, `agingReportRespectsAsOfDate` (`PartyFinancialPositionServiceTests.java`). See `docs/DEEP_ENGINEERING_REVIEW_2026-09-06.md` §"Remediation Status" for full evidence. The original finding above is left unedited as the historical record of what was found and when.
...
> **UPDATE (2026-09-07, Critical Engineering Remediation):** This gap is now fixed. All three controllers carry a real, differentiated `@PreAuthorize` role matrix (not a blanket `SUPER_ADMIN`-only gate) — class-level `SUPER_ADMIN`/`ADMIN`/`SALES_MANAGER`/`INVENTORY_MANAGER`/`GENERAL_MANAGER` for reads, a tighter write set excluding `GENERAL_MANAGER`, and `WorkOrderController.deliverAndCreateInvoice` (the invoice-generating action) further excludes `INVENTORY_MANAGER`. Proven by a new `ServiceOpsAuthorizationIntegrationTests` class that executes real Spring Security through real HTTP requests (not `check-authorization-contract.py`, per this document's own caution above about what that script cannot see): authorized role → 2xx, disallowed authenticated role → 403, unauthenticated → 401. Building and running this real test also surfaced and fixed a separate, previously-undetected defect: `BookableResource`/`RentalItem`/`RentalContract`/`WorkOrder` all explicitly set `version = 0L` in their constructors, which defeated Spring Data JPA's insert-vs-update detection and made every create call to these three controllers fail with an opaque `409 CONCURRENT_MODIFICATION` — see `docs/DEEP_ENGINEERING_REVIEW_2026-09-06.md` §"Remediation Status" for full evidence. The original finding above is left unedited as the historical record.
...
1. ~~**AR aging as-of-date is accepted but ignored**~~ — **FIXED 2026-09-07** (Critical Engineering Remediation). `PartyFinancialPositionService.getFinancialPosition()` now honors `asOfDate`; see §9.5's update note. Original text preserved: *`PartyFinancialPositionService.getFinancialPosition()` always buckets against `System.currentTimeMillis()`; the `asOfDate` request parameter has no effect on which aging bucket a receivable lands in. No test covers this path.* (§9.5)
2. ~~**`serviceops` bookings/rentals/work-orders have no authorization annotations at all**~~ — **FIXED 2026-09-07** (Critical Engineering Remediation). All three controllers now carry a real, differentiated `@PreAuthorize` role matrix, proven by real Spring-Security-executing tests; see §10.1's update note. Original text preserved: *any authenticated user of any role can create/cancel bookings, activate/close/cancel rental contracts, and create/deliver work orders (the latter generates a customer invoice). Tenant isolation is intact; this is a role/permission gap.* (§10.1)
