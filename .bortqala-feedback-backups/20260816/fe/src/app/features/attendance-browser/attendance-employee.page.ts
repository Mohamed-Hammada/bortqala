...
export class AttendanceEmployeePage {
  ...
  async loadCategories(): Promise<void> {
    ...
    try {
      ...
      // BORTQALA_CORRECTIVE_20260816_EMPTY_CATEGORY_REDIRECT
      if (allowed.length === 0) {
        this.notification.warning(this.i18n.locale() === 'ar-EG'
          ? 'لا توجد فئة حضور متاحة. أضف فئة حضور أولاً ثم ارجع لإضافة الموظف.'
          : 'No attendance category is available. Create an attendance category first, then return to add the employee.');
