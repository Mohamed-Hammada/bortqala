...
- Features: dashboard, dynamic categories/rules and advance eligibility, generated category-prefixed employee codes, employee/device mapping, biometric CSV/XLS/XLSX/MDB/ACCDB imports, downloadable templates on every structured-data import surface, custom report ranges, translated review, business parties, inventory/signed ledgers/advances, multi-role users, top-corner settings, light/dark/system theme, database i18n initialized before rendering, SVG icons, pagination, ESC drawers, and localized Excel exports.
...
- The operations workbench includes valuation evidence, reorder alerts, per-item replenishment settings, and idempotent cycle-count entry/history; all balance, variance, valuation, and audit calculations remain backend-owned.
- The operations workbench also lists warehouse transfers and supports creating a one-item draft plus ship, receive, and cancel actions. The backend remains authoritative for transfer state, source-stock validation, balance movement, idempotency, and audit evidence.
- Select future work from verified source gaps and failing acceptance tests. Treat README/roadmap status labels as historical hints, not proof that a feature exists or works.
